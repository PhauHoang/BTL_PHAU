...
namespace botPhau
{
    ...
    public partial class bot : Form
    {
        ...
        {
            if (txtbot.InvokeRequired)
            {
                txtbot.BeginInvoke((MethodInvoker)delegate ()
                {
                    AddLog(msg);
                });
            }
        ...
        public bot()
        {
            ...
            string token = "6030227114:AAGzpMQ0Hx3yCaXYb7otNEVRIpcEfPHVXCE";
            ...
            async Task HandleUpdateAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
            {
                ...
                else if (messLow.StartsWith("tksv "))
                {

                    string tenSV = messageText.Substring(5);
                    //timkiemsv tk = new timkiemsv();
                    //reply = tk.Timkiemsv("%" + tenSV.Replace(' ', '%') + "%");
                }
                else // Nếu k phải là thằng nào đặc biệt thì => hát cho Pạn nghe
                {
                    reply = "Sếp vừa nói😱: " + messageText;
                }
                //if (messLow.StartsWith("ttsv"))
               // {
               ////     reply=""
               // }    
                    // ----------- KẾT THÚC XỬ LÝ -----------------------------------------------------------------------
